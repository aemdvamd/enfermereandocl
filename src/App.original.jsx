...
function RequestForm({ services = [], onSubmit, onCancel }) {
  ...
  const validateForm = () => {
    ...
    beneficiaries.forEach((ben, bIndex) => {
      if (!ben.name?.trim()) {
        newErrors[`beneficiary-${bIndex}-name`] = 'El nombre del beneficiario es obligatorio';
      }
  ...
  // Actualizar servicio de un beneficiario
  const updateService = (bIndex, sIndex, field, value) => {
    const updated = beneficiaries.map((ben, i) => {
      if (i !== bIndex) return ben;
      const newServices = ben.services.map((srv, j) => 
        j === sIndex ? { ...srv, [field]: value } : srv
      );
  ...
  // Agregar servicio a un beneficiario
  ...
  // Eliminar servicio de un beneficiario
  const removeServiceFromBeneficiary = (bIndex, sIndex) => {
    const updated = beneficiaries.map((ben, i) => {
      if (i !== bIndex) return ben;
      return {
        ...ben,
        services: ben.services.filter((_, j) => j !== sIndex)
      };
    });
    setBeneficiaries(updated);
  };
    ...
    <div className="bg-white rounded-3xl shadow p-8 max-w-4xl mx-auto">
      ...
      <form onSubmit={handleSubmit} className="space-y-10">
          ...
          <div key={ben.id} className="border border-gray-200 rounded-3xl p-6">
            <div className="flex justify-between items-center mb-6">
              ...
              {beneficiaries.length > 1 && (
                <button
                  type="button"
                  onClick={() => removeBeneficiary(bIndex)}
            ...
            <div className="mt-8">
              ...
              {ben.services.map((srv, sIndex) => (
                <div key={sIndex} className="flex gap-4 items-end mb-4">
                  ...
                  {ben.services.length > 1 && (
                    <button
                      type="button"
                      onClick={() => removeServiceFromBeneficiary(bIndex, sIndex)}
              ...
              <button
                type="button"
                onClick={() => addServiceToBeneficiary(bIndex)}
        ...
        <button
          ...
          className="w-full py-4 border border-dashed border-gray-300 rounded-3xl text-gray-600 hover:border-indigo-300 hover:text-indigo-600 transition-colors"
        >
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div>
            ...
            <input
              ...
              className={`w-full px-5 py-4 rounded-3xl border ${errors.date ? 'border-red-500' : 'border-gray-300'}`}
            />
          ...
          <div>
            ...
            <input
              ...
              className={`w-full px-5 py-4 rounded-3xl border ${errors.time ? 'border-red-500' : 'border-gray-300'}`}
            />
          ...
          <div>
            ...
            <select
              ...
              className="w-full px-5 py-4 rounded-3xl border border-gray-300 focus:border-indigo-500"
              >
        ...
        <button
          type="submit"
          className="w-full bg-indigo-600 hover:bg-indigo-700 text-white py-5 rounded-3xl text-xl font-semibold transition-all"
        >
          Enviar Solicitud
        </button>
      </form>
...
function AdminPanel({ 
  ...
  const [tab, setTab] = useState('servicios'); // pestaña por defecto
  ...
  const addNewService = () => {
    if (!newServiceName.trim()) return alert('Debes ingresar un nombre');
  ...
  const updateService = (id, field, value) => {
    const updated = safeServices.map(s => s.id === id ? { ...s, [field]: value } : s);
    setServices(updated);
  };
  ...
  return (
    <div className="max-w-7xl mx-auto p-6">
      <div className="flex justify-between items-center mb-8">
        ...
        <button onClick={() => setView('landing')} className="flex items-center gap-2 text-gray-500 hover:text-gray-700">← Cerrar sesión</button>
      </div>
              ...
              <div key={service.id} className="bg-white border border-gray-200 rounded-3xl p-6 hover:shadow-xl transition-all">
                ...
                <div className="mt-8 flex gap-3">
                  <button 
                    ...
                    className="flex-1 text-red-500 hover:text-red-700 text-sm font-medium py-3 border border-red-200 rounded-3xl hover:bg-red-50"
                  >
                    Eliminar
                  </button>
                </div>
      ...
      {/* Otras pestañas (puedes mantenerlas) */}
      {tab === 'solicitudes' && <div className="bg-white rounded-3xl p-8 text-gray-500">Lista de solicitudes...</div>}
      {tab === 'seguimiento' && <div className="bg-white rounded-3xl p-8 text-gray-500">Seguimiento de dosis...</div>}

      <div className="mt-12 text-center text-xs text-gray-400">Admin Panel • Enfermereando © 2026</div>
    </div>
  );
}
